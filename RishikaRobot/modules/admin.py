...
from RishikaRobot import DRAGONS, dispatcher,OWNER_ID,pbot
from RishikaRobot.modules.disable import DisableAbleCommandHandler
from RishikaRobot.modules.helper_funcs.admin_rights import user_can_changeinfo
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"❖ sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇᴍᴏᴛᴇᴅ ᴀ ᴀᴅᴍɪɴ ɪɴ <b>➥ {chat.title}</b>\n\n● ᴅᴇᴍᴏᴛᴇᴅ ᴜsᴇʀ ➥ <b>{mention_html(user_member.user.id, user_member.user.first_name)}</b>\n● ᴅᴇᴍᴏᴛᴇʀ ʙʏ ➥ {mention_html(user.id, user.first_name)}",
        parse_mode=ParseMode.HTML,
        )
        ...
        message.reply_text(
            "✦ ғᴀɪʟᴇᴅ ᴛᴏ ᴅᴇᴍᴏᴛᴇ ᴍᴀʏʙᴇ ɪ'ᴍ ɴᴏᴛ ᴀɴ ᴀᴅᴍɪɴ ᴏʀ ᴍᴀʏʙᴇ sᴏᴍᴇᴏɴᴇ ᴇʟsᴇ ᴩʀᴏᴍᴏᴛᴇᴅ ᴛʜᴀᴛ ᴜsᴇʀ !",
        )
        return
